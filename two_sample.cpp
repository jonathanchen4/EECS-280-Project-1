    ...
    vector<double> data_A, vector<double> data_B)
{
  ...
  for (int i = 0; i < 1000; i++)
  {
    ...
    double result = mean(sample_A) - mean(sample_B);
...
pair<double, double> confidence_interval(vector<double> v, double width)
{
  ...
  double upper_bound = (1 - lower_bound);

  double lower_percent = percentile(v, lower_bound);
  double upper_percent = percentile(v, upper_bound);
